...
const getItemsBySubCategoryId = async (req , res) => {
    try {
        const items = await Item.find({ parentId : req.params.id , parentModel : ParentModel.SubCategory});
        return res.status(200).json(items);
    } catch (error) {
        return res.status(400).json({ error: error.message });
    }
...
const getItemsByCategoryId = async (req , res) => {
    try {
        ...
        const items = await Item.find({ parentId : { $in : subCategory.map(sub => sub._id)} , parentModel : ParentModel.SubCategory});
        return res.status(200).json(items);
    }
    catch (error) {
        return res.status(400).json({ error: error.message });
